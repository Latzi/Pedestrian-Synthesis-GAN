...
class AlignedDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        ...
        AB = AB.resize((w_total, h), Image.BICUBIC)
        ...
        bbox_list = [bbox_y, bbox_x, bbox_w, bbox_h]
        ...
        if (not self.opt.no_flip) and random.random() < 0.5:
            ...
            bbox_list = [bbox_list[0], A.size(2) - bbox_list[2], A.size(2) - bbox_list[1], bbox_list[3]]
            #print(f"-----------Image at index {index} flipped.------------------")
        #else:
            #print(f"No flipping for index {index}")

        return {'A': A, 'B': B, 'bbox': bbox_list, 'A_paths': AB_path, 'B_paths': AB_path}
